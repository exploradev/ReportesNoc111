...
module.exports = function (app, io) {
  ...
  app.post("/get_userlist_activos", middleware.requireLogin, function (req, res) {
    let query = "SELECT * from users WHERE estatus = 'activo'";
  ...
  app.post("/get_userlist_inactivos", middleware.requireLogin, function (req, res) {
    let query = "SELECT * from users WHERE estatus = 'inactivo'";
    connection.getConnection(function (err, conn) {
      conn.query(query, function (error, results, field) {
        if (error) throw error;
        res.send(results);
  ...
  app.post("/guardar_nuevousuario",middleware.requireLogin,function (req, res) {
      ...
      connection.getConnection(function (err, pool) {
        pool.beginTransaction(function (err) {
          ...
          var query =
            "INSERT INTO users(nombre,username,password,rol,estatus,turno) VALUES(?,?,?,?,?,?)";
  ...
  app.post("/getSingleUser", middleware.requireLogin, function (req, res) {
    ...
    var query =
      "SELECT iduser,nombre,username,rol,turno,estatus from users where iduser = ? LIMIT 1";
    var inserts = [iduser];
    connection.getConnection(function (err, conn) {
      query = mysql.format(query, inserts);
      conn.query(query, function (error, results, field) {
  ...
  app.post("/actualizar_usuario", middleware.requireLogin, function (req, res) {
    ...
    if (password == "sincambios") {
      var query =
        "UPDATE users SET nombre = ?, username = ?, rol = ?, estatus = ?, turno = ? WHERE iduser = ?";
      var inserts = [nombre, username, rol, estatus, turno, iduser];
      query = mysql.format(query, inserts);
    } else {
      var query =
        "UPDATE users SET nombre = ?, username = ?, password = ?, rol = ?, estatus = ?, turno = ? WHERE iduser = ?";
  ...
  app.post("/getReport", middleware.requireLogin, function (req, res) {
    ...
    if (tipo == "global") {
        ...
        conn.query(query, function (error, results, field) {
          ...
          results.forEach((element) => {
            ...
            if (element.ultseguimiento) {
              element.ultseguimiento = moment(element.ultseguimiento).format(
                "DD[/]MM[/]YYYY[ ]HH[:]mm[:]ss"
              );
            }
            if (element.cerrado) {
              element.cerrado = moment(element.cerrado).format(
                "DD[/]MM[/]YYYY[ ]HH[:]mm[:]ss"
              );
            }
            if (element.fecha_asignacion) {
              element.fecha_asignacion = moment(element.fecha_asignacion).format(
                "DD[/]MM[/]YYYY[ ]HH[:]mm[:]ss"
              );
            }
          });
      ...
      var query =
        "SELECT * FROM ?? aux LEFT JOIN vista_metadatos v on v.idmetadatos = aux.idmetadatos where v.creado between date(?) and date(?)";
      var inserts = [tipo, desde, hasta];
      query = mysql.format(query, inserts);
      connection.getConnection(function (err, conn) {
        conn.query(query, function (error, results, field) {
  ...
  app.post("/getTipificacion", middleware.requireLogin, function (req, res) {
    ...
    var query =
      "SELECT tipificacion from metadatos where idmetadatos = ? LIMIT 1";
  ...
  app.post("/getTables", middleware.requireLogin, function (req, res) {
    var query =
      "SELECT tabla FROM tablasporexportar WHERE activa IN ('activa','falla1','falla2','falla3')";
  ...
  app.post("/generartabla", middleware.requireLogin, function (req, res) {
    ...
    var query_head =
      "CREATE TABLE ?? (id int(11) NOT NULL AUTO_INCREMENT,asesor varchar(200) DEFAULT NULL,ipaddr varchar(45) DEFAULT NULL, creado timestamp NULL DEFAULT CURRENT_TIMESTAMP,";
    var query_body = "";
    var query_footer =
      " PRIMARY KEY (id)) ENGINE = InnoDB DEFAULT CHARSET = utf8";

    //DEFINO LA VARIABLE DEL CONTENIDO
    var contentHTMLheader =
      '<div class="group-masivas"> <p class="titulocampo">';
    var contentHTMLfooter =
      '</p> <input class="inputsasesor campo_fallamasiva"/></div>';
    ...
    connection.getConnection(function (err, conn) {
      conn.query(query, function (error, results, field) {
        if (error) {
          ...
          var ingresar =
            "INSERT INTO tablasporexportar(tabla,activa,tipo,descripcion,autor,contenido) VALUES(?,'inactivo','dinamica',?,?,?)";
          var variables = [
            parametros[1],
            parametros[2],
            parametros[0],
  ...
  app.post("/getTablesFallas", middleware.requireLogin, function (req, res) {
    var query =
      " SELECT t.idtablasporexportar,t.tabla,t.activa,t.creado,t.descripcion,u.nombre FROM tablasporexportar t LEFT JOIN users u ON u.iduser = t.autor WHERE tipo = 'dinamica' ";
  ...
  app.post("/activar_tabla", middleware.requireLogin, function (req, res) {
    ...
    var query_desactivar =
      "UPDATE tablasporexportar SET activa = 'inactivo' WHERE tipo = 'dinamica' and activa = ?";
    var insercion = [numero];
    query_desactivar = mysql.format(query_desactivar, insercion);

    var query_activar =
      "UPDATE tablasporexportar SET activa = ? WHERE tipo = 'dinamica' AND idtablasporexportar = ?";
  ...
  app.post("/desactivar_tabla", middleware.requireLogin, function (req, res) {
    ...
    var query_desactivar =
      "UPDATE tablasporexportar SET activa = 'inactivo' WHERE tipo = 'dinamica' and idtablasporexportar = ?";
    var insercion = [id];
    query_desactivar = mysql.format(query_desactivar, insercion);
  ...
  app.post(
    ...
    function (req, res) {
      var query_desactivar =
        "UPDATE tablasporexportar SET activa = 'inactivo' WHERE tipo = 'dinamica'";
  ...
  app.post(
    "/validarFallasActivas",
    middleware.requireLogin,
    function (req, res) {
      var query =
        "SELECT activa FROM tablasporexportar WHERE activa in ('falla1','falla2','falla3') and tipo = 'dinamica'";
      connection.getConnection(function (err, conn) {
        conn.query(query, function (error, results, field) {
          if (error) {
  ...
  app.post(
    "/getTablaMasivaActiva",
    middleware.requireLogin,
    function (req, res) {
      var query =
        " SELECT descripcion, contenido FROM tablasporexportar WHERE tipo = 'dinamica' and activa = ? order by creado desc limit 1 ";
  ...
  app.post(
    ...
    function (req, res) {
      ...
      var query_getActiva =
        "SELECT tabla FROM tablasporexportar WHERE tipo = 'dinamica' AND activa = ? order by creado desc LIMIT 1";
          ...
          if (error) {
            ...
            conn.query(query_getNames, function (error, results, field) {
              if (error) {
                ...
                console.log(parametros);
                var query_header =
                  "INSERT INTO " + nombre_tabla + "(ipaddr,asesor";
                //------------------------------------------------------
                var query_body1 = "";
                ...
                //--------------------------------------------------------
                var finalquery =
                  query_header + query_body1 + query_body2 + query_footer;
                ...
                conn.query(finalquery, function (error, results, field) {
                  if (error) {
                    ...
                    res.send("Correcto");
                    console.log(
                      "AGREGADO CORRECTAMENTE A SU TABLA DE FALLA MASIVA"
                    );
                  }
                });
                //FIN DE INSERCION
