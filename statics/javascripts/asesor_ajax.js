...
$(document).ready(function () {
  ...
  reset_all = function () {
    ...
    $("#telefono_afectado_internetencasa").html("");
          ...
          if (response == "existe") {
            ...
            switch (tiporeporte) {
              ...
              case "callback":
                ...
                break;
              case "internetencasa":
                $("#captura_tipo").html("Internet en casa");
                $("#telefono_afectado_ineternetencasa").html(telefono);
                $("#body_modal_capturaineternetencasa").css("display", "grid");
                $("#container_footer_ineternetencasa").css("display", "flex");
                break;
            }
          }
          //ESCONDER LOADER
  ...
  $("#btn_captura_cobertura").click(function () {
    ...
    if (acumulador_inputs_erroneos != 0 || acumulador_por_llenar != 0) {
      ...
      alert(
        "Es necesario llenar todos los campos correctamente antes de guardar \n Por llenar: " +
          acumulador_por_llenar +
          "\n Con error: " +
          acumulador_inputs_erroneos
      );
  ...
  $("#btn_captura_internetencasa").click(function () {
    var acumulador_no_vacios = 0;
    var conteo_inputs = 0;
    var acumulador_por_llenar = 0;
    var acumulador_inputs_correctos = 0;
    var acumulador_inputs_erroneos = 0;
    //----------------------------------
    $(".inputs_internetencasa").each(function () {
      //lleno mi contador de inputs
      conteo_inputs++;
    ...
    if (acumulador_inputs_erroneos != 0 || acumulador_por_llenar != 0) {
      ...
      //get valor telefono
      var iduser = $("body").data("iduser");
      var telefono_afectado = $("#telefono_afectado_ineternetencasa").html();

      $(".inputs_internetencasa")
        .closest(".has-success")
        .children(".inputs_internetencasa")
        .each(function () {
          parametro_ajax.push($(this).val());
        });
      ...
      $.post(
        "/guardar_internetencasa",
        {
          ...
          nombreusuario: parametro_ajax[0],
          contacto: parametro_ajax[1],
          fecha: parametro_ajax[2],
          escenario: parametro_ajax[3],
          descripcion: parametro_ajax[4],
          domicilio: parametro_ajax[5],
          estado: parametro_ajax[6],
          municipio: parametro_ajax[7],
          colonia: parametro_ajax[8],
          cp: parametro_ajax[9],
        ...
        if (response == "no existe") {
          ...
          for (i = 0; i < response.length; i++) {
            ...
            if (response[i]["cerrado"] == null) {
              ...
              table_body += "<td>";
              table_body += moment(response[i]["cerrado"]).format(
                "DD/MM/YYYY HH:mm"
              );
              table_body += "</td>";
            }
  ...
  $("#btn_sendreferido").click(function () {
    ...
    if (numero != "") {
      ...
      alert("Ingresar número a 10 dígitos");
    }
  });
  ...
  $(".hidethisradios").change(function () {
    ...
    if (value == "rojo") {
      alert(
        "No es posible enviar este número ya que esta dado de alta en lista negra"
      );
    }
  });
});
