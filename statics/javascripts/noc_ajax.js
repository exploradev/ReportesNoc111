...
$(document).ready(function () {
  ...
  $("#boton_enproceso").click(function () {
    ...
    $.post(
      "/setPrimerBloqueo",
      { iduser: iduser, idmetadatos: idmetadatos, estatus: "En proceso" },
      function (response) {
        if (response == "En proceso") {
          ...
          $("#estatus_seguimiento_solucionado")
          ...
          $('.clickable_filter[data-filter="' + panelabierto + '"]').trigger(
            "click"
          );

          $.post(
            "/get_detallesmetadatos",
      ...
      $.post(
        ...
        function (response) {
          if (response == "Solucionado") {
            ...
            //reload tabla actual trtgger click de panel abierto
            var panelabierto = $("#tiporeporte_header").attr(
              "data-filterclick"
            );
            $('.clickable_filter[data-filter="' + panelabierto + '"]').trigger(
              "click"
            ...
            $.post(
              ...
              function (inner_response) {
                ...
                var newMoment = moment(servertime).format(
                  "YYYY-MM-DD HH:mm:ss"
                );
                servertime = new Date(newMoment);
                clearInterval(clockInterval); //reseteo el contador de la ventana actual
          ...
          } else if (response == "Cerrado" || response == "Rechazado") {
            //si el estatus es cerrado o rechazado
    ...
    $.post(
      ...
      function (response) {
        ...
        for (i = 0; i < response.length; i++) {
          ...
          table_body += "<td>";
          table_body += moment(response[i]["creado"]).format(
            "DD/MM/YYYY HH:mm"
          );
          table_body += "</td>";
  ...
  $("#tbody_maintable").on("click", ".rowdetallesdisponibles", function () {
    ...
    switch (tipofalla) {
      ...
      case "callback":
        ...
        break;
      case "internetencasa":
        $("#body_modal_detallesinternetencasa").css("display", "grid");
        break;
    }
    ...
    $.post(
      ...
      function (response) {
        ...
        if (response[0]["estatus"] == "En proceso") {
          ...
          response[0]["estatus"] == "Cerrado" ||
          response[0]["estatus"] == "Rechazado"
        ) {
          //ocultar selects y botones
          $("#form_seguimiento").show();
          $("#boton_enproceso ").hide();
        ...
        $("#seccion_comentarioslibres").attr(
          "data-estatus",
          response[0]["estatus"]
        );
        ...
        switch (tipofalla) {
          case "aclaraciones":
            ...
            break;
          case "general":
            $("#detalles_metadatospanel_tiporeporte").html(
              "Afectación general"
            );
            break;
          case "cobertura":
          ...
          case "callback":
            ...
            break;
          case "internetencasa":
            $("#detalles_metadatospanel_tiporeporte").html("Internet en casa");
            break;
        }
        $("#detalles_metadatospanel_asesor").html(response[0]["nombre"]);
        $("#detalles_metadatospanel_creado").html(
          moment(response[0]["creado"]).format("DD/MM/YYYY HH:mm")
        );
        $("#detalles_metadatospanel_tipificacion").html(
          response[0]["tipificacion"]
        );
      ...
      function (response) {
        switch (tipofalla) {
          ...
          case "callback":
            ...
            break;
          case "internetencasa":
            filldetalles_internetencasa(response);
            break;
        }
  ...
  filldetalles_aclaraciones = function (response) {
    ...
    $("#detalle_aclaraciones_descripcionsituacion").html(
  ...
  filldetalles_cobertura = function (response) {
    ...
    $("#detalle_cobertura_fechanacimiento").html(
      response[0]["fechanacimiento"]
    );
    $("#detalle_cobertura_lugarnacimiento").html(
      response[0]["lugarnacimiento"]
    );
  ...
  filldetalles_navegacion = function (response) {
    ...
    $("#detalle_navegacion_fechayhorainiciofalla").html(
      response[0]["fechahora"]
    );
  ...
  filldetalles_internetencasa = function (response) {
    $("#detalle_internetencasa_telefono").html(response[0]["telefono"]);
    $("#detalle_internetencasa_nombreusuario").html(response[0]["usuario"]);
    $("#detalle_internetencasa_contacto").html(response[0]["contacto"]);
    $("#detalle_internetencasa_fecha").html(response[0]["fecha"]);
    $("#detalle_internetencasa_escenario").html(response[0]["escenario"]);
    $("#detalle_internetencasa_descripcion").html(response[0]["descripcion"]);
    $("#detalle_internetencasa_domicilio").html(response[0]["domicilio"]);
  };
  ...
  timestampsCounter = function (server_time) {
    ...
    var current =
      dias + " dias, " + horas + ":" + minutos + ":" + segundos + " hrs";
    ...
    clockInterval = setInterval(function () {
      ...
      if (horas < 10) {
        if (toString(horas).charAt(0) != "0") {
          horas = "0" + horas;
        }
      }
      if (minutos < 10) {
        if (toString(minutos).charAt(0) != "0") {
          minutos = "0" + minutos;
        }
      }
      if (segundos < 10) {
        if (toString(segundos).charAt(0) != "0") {
          segundos = "0" + segundos;
        }
      }
      var current =
