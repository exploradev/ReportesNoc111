...
$(document).ready(function () {
  ...
  construirRelojEn = function (selector, server_time) {
    ...
    if (horas > 1 && horas < 4 && dias == 0) {
      $("#" + selector).html(
        "<span style='color:orange;font-weight:bold;'>" + current + "</span>"
      );
  ...
  llenar_conteos_todos = function () {
    $.get("/get_conteosmenu", function (response) {
      ...
      $("#conteo_internetencasa").html(response[0]["conteo_internetencasa"]);
      ...
      $("#pendientes_aclaraciones").html(
        response[0]["pendientes_aclaraciones"]
      );
      ...
      $("#abiertos_internetencasa").html(
        response[0]["abiertos_internetencasa"]
      );
      $("#nuevos_internetencasa").html(response[0]["nuevos_internetencasa"]);
      $("#enproceso_internetencasa").html(
        response[0]["enproceso_internetencasa"]
      );
      $("#pendientes_internetencasa").html(
        response[0]["pendientes_internetencasa"]
      );
      $("#cerrados_internetencasa").html(
  ...
  $(".clickable_filter").click(function () {
    ...
    if (filtro != "cerradosrechazados") {
      ...
      if (mios == "show") {
        ...
        $.post(
          "/get_maintabledata_propios",
          { filtro: filtro, iduser: iduser },
          function (response) {
            var table_body = [];
            for (i = 0; i < response.length; i++) {
              table_body +=
                "<tr id='row" +
                response[i]["idmetadatos"] +
                "' class='rowdetallesdisponibles' data-idmetadatos='" +
                response[i]["idmetadatos"] +
                "' data-tipofalla='" +
                response[i]["falla"] +
                "'>";
              ...
              if (response[i]["ultseguimiento"] == null) {
                ...
                table_body += "<td>";
                table_body += moment(response[i]["ultseguimiento"]).format(
                  "DD/MM/YYYY HH:mm"
                );
                table_body += "</td>";
              }
              ...
              } else if (response[i]["falla"] == "internetencasa") {
                table_body += "Internet en casa";
              } else {
                table_body += "N/A";
              }
              table_body += "</td>";
              ...
              if (ultimoestatus == "Cerrado" || ultimoestatus == "Rechazado") {
                ...
                $("#clock_nuevo_" + idreporte + numero_random)
                  .empty()
                  .remove();

                table_body +=
                  '<td id="clock_nuevo_' + idreporte + numero_random + '">';
                table_body += "--";
                table_body += "</td>";
                var row = {
                  ["clock_nuevo_" + idreporte + numero_random]: response[i][
                    "creado"
                  ],
                };
                ...
                var numero_random = Math.random() * (999999 - 000000) + 000000;
                numero_random = parseInt(numero_random);
                $("#clock_enproceso_" + idreporte)
                  .empty()
                  .remove();
                table_body +=
                  '<td id="clock_enproceso_' + idreporte + numero_random + '">';
                table_body += "--";
                table_body += "</td>";

                var row = {
                  ["clock_enproceso_" + idreporte + numero_random]: response[i][
                    "enproceso_time"
                  ],
                };
                lista_relojes.push(row);
                ...
                var numero_random = Math.random() * (999999 - 000000) + 000000;
                numero_random = parseInt(numero_random);
                $("#clock_solucionado_" + idreporte)
                  .empty()
                  .remove();
                table_body +=
                  '<td id="clock_solucionado_' +
                  idreporte +
                  numero_random +
                  '">';
                table_body += "--";
                ...
                var row = {
                  ["clock_solucionado_" + idreporte + numero_random]: response[
                    i
                  ]["solucionado_time"],
                };
                lista_relojes.push(row);
          ...
          for (i = 0; i < response.length; i++) {
            ...
            if (ultimoestatus == "Cerrado" || ultimoestatus == "Rechazado") {
              ...
              $("#clock_solucionado_" + idreporte)
                ...
                .remove();
              table_body +=
                '<td id="clock_solucionado_' + idreporte + numero_random + '">';
              table_body += "--";
              table_body += "</td>";

              var row = {
                ["clock_solucionado_" + idreporte + numero_random]: response[i][
                  "solucionado_time"
                ],
              };
              lista_relojes.push(row);
